  ...
  if [[ "$OSTYPE" == "linux-gnu"* ]]; then
    ...
    if [[ "$NAME" == "Manjaro Linux" ]]; then
      ...
      yay -S --needed neofetch tmux tealdeer ntfs-3g xclip vscodium-bin \
        ...
        shellcheck plex-media-player google-cloud-sdk quick-lookup remmina \
      ...
      sudo apt update
      sudo apt install -y zsh neofetch tmux htop tldr fzf python3 trash-cli zoxide ripgrep batcat
    fi
